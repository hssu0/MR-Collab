...
namespace MRTK.Tutorials.MultiUserCapabilities
{
    public class PhotonRoom : MonoBehaviourPunCallbacks, IInRoomCallbacks
    {
        ...
        [SerializeField] private GameObject puzzlePrefab = default;
        ...
        private void Start()
        {
            ...
            if (PhotonNetwork.PrefabPool is DefaultPool pool)
            {
                ...
                if (puzzlePrefab != null) pool.ResourceCache.Add(puzzlePrefab.name, puzzlePrefab);
            }
        }
        ...
        private void CreateInteractableObjects()
        {
            ...
            var positionOnTopOfSurface_rover = new Vector3(position.x, position.y + roverExplorerLocation.localScale.y / 2,
                position.z);
            var positionOnTopOfSurface_puzzle = new Vector3(position.x - 1f, position.y + roverExplorerLocation.localScale.y + 0.8f,
                position.z - 0.25f);


            var go = PhotonNetwork.Instantiate(roverExplorerPrefab.name, positionOnTopOfSurface_rover, roverExplorerLocation.rotation);
            var puzzle = PhotonNetwork.Instantiate(puzzlePrefab.name, positionOnTopOfSurface_puzzle, roverExplorerLocation.rotation);
        }
